...
def Inv_Mix_Columns(state):
    step = 0 #step for selecting four bytes at a time
    
    for i in range(4):
        
        t1 = Mult_0e[state[step]] ^ Mult_0b[state[step + 1]] ^ Mult_0d[state[step + 2]] ^ Mult_09[state[step + 3]]
        t2 = Mult_09[state[step]] ^ Mult_0e[state[step + 1]] ^ Mult_0b[state[step + 2]] ^ Mult_0d[state[step + 3]]
        t3 = Mult_0d[state[step]] ^ Mult_09[state[step + 1]] ^ Mult_0e[state[step + 2]] ^ Mult_0b[state[step + 3]]
        t4 = Mult_0b[state[step]] ^ Mult_0d[state[step + 1]] ^ Mult_09[state[step + 2]] ^ Mult_0e[state[step + 3]]
...
def Decrypt(plain_bytes, round_keys):
    decrypted_blocks = []
    for state in plain_bytes:
...
decrypted_bytes = Decrypt(plain_bytes,round_keys)


byte_list = [];
for i in range(len(decrypted_bytes)):
    byte_list += decrypted_bytes[i] 

del byte_list[-pad_length : ]
byte_array = bytearray(byte_list)


restored_img = Image.frombytes(img_mode, img_size, byte_array)
restored_img.save('restored_image1.png')
